...
class DFsToExcel:
    ...
    def export_dfs_to_excel(self):
        ...
        if 'hw_ave_pose_df' in self.sequence_dict:
            self.ex_print.sheet_name = 'hw_ave_pose_df'
            df = self.sequence_dict['hw_ave_pose_df']
            self.print_df(df, 'hw_ave_pose_df')
        if 'top_weights_pose_df' in self.sequence_dict:
            self.ex_print.sheet_name = 'top_weights_pose_df'
            df = self.sequence_dict['top_weights_pose_df']
            self.print_df(df, 'top_weights_pose_df')
